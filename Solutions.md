...
- For testing status messages:
  ...
  - Run
  ```
  curl -XPOST --data "data=blah" https://mysterious-hollows-73808.herokuapp.com/api/messages
  ```
  to post a message
...
- Examples for performing CRUD methods on our API:
  ...
  - PUT method:
    ...
    ```
    // This put method updates the message's read flag attribute to true after 
    // the user abc@mail.com has read the message with id = 0
